...
\subsection{UAV model}\todo{Subsection ne bi trebao poceti odmah nakon sectiona}
...
\end{equation}
where $\text{J}_b \in \mathbb{R}^3$ is body and $\text{J}_i \in \mathbb{R}^3$ is \todo{the} moment of inertia of some mass element outside the origin of the body-fixed frame. Using the parallel axis theorem, one is able to calculate $\text{J}_i$ while knowing \todo{the} moment of inertia around its CoG:
\begin{equation}
...
It is important to note that the changes in \todo{the} moment of inertia and CoG have been omitted in favor of simplicity in equations \eqref{model2} and \eqref{model4}. The complete model dynamics{\tiny } expressed in the inertial frame are presented in \ref{sec:appendix}.
\begin{gather}
...
\indent Height and yaw of the UAV is controlled by \todo{with} variations in rotor velocity, whereas roll and pitch by \todo{with} variations in CoG. It is assumed that \todo{the} first and \todo{the} third propeller rotate clockwise, while \todo{the} second and \todo{the} fourth rotate counter-clockwise. The relation between moments, thrust and rotor velocity is the following:
\begin{gather}
...
\end{equation}
\\
Manipulator and moving mass actuator dynamics along with the change in desired rotor force is regarded as instantaneous while presenting the controller synthesis and stability conditions. However, within the Gazebo simulation environment, a certain transfer dynamic is taken in\todo{into}  account.