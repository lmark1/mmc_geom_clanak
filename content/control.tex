...
Compatible attitude error function and transport map between tangent bundles of SO(3) are chosen as suggested in \cite{bulloBook} and confirmed in research regarding geometric control with aerial vehicles \cite{LeeClanak4}, \cite{LeeClanak3}, \cite{LeeClanak1}, \cite{LeeClanak2}. \todo{1.6} Attitude error function on SO(3) $	\Psi(\text{R}, \text{R}_d)$  along with its compatible transport map $\Ta(\text{R}, \text{R}_d)$ are chosen as:
\begin{gather}
...
\begin{equation}
	\begin{split}
		& \lVert \, m_tge_3 + m\ddot{\textbf{x}}_d - m_t\text{R} \,(\, \textbf{r}_{CoG}  \times \dot{\mb{\Omega}} \,) \\
		& - m_t \text{R} \,[\, \mb{\Omega} \times (\, \textbf{r}_{CoG} \times \mb{\Omega} \,)\, ] \, \rVert < B \, , \label{condition2}
	\end{split}
\end{equation}
